...
export const getBreadCrumbItems = (path: any, name?: string) => {
    const items = [];
    ...
    for (let i = 0; i < label.length; i++) {
        items.push({
            label: !isNaN(label[i + 1]) ? name : label[i + 1],
            url: i === 0 ? '/' + label[i + 1] : '/' + label[i] + '/' + label[i + 1],
        });
    }
...
export const getBannerTypeOptions = () => {
    return [
        ...
        { value: 'CAR', label: 'Cars page' },
        { value: 'ABOUT_US', label: 'About-us page' },
        { value: 'BLOG', label: 'Blogs page' },
        { value: 'CONTACT_US', label: 'Contact us page' },
        { value: 'PRE_ORDER', label: 'Pre-order page' },
        { value: 'VERIFY_AUCTION', label: 'Verify-auction page' },
    ];
};

export const getIsFeaturedOptions = () => [
